...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    /**
     * @notice Set a vault wallet address for handler contracts that use the IERCHandler interface.
     * @notice Only callable by an address that currently has the admin role.
     ...
     */
    function getVaultProposal(
        uint8 originChainID,
        uint64 depositNonce,
        bytes32 dataHash
    ) external view returns (VaultProposal memory) {
        uint72 nonceAndID = (uint72(depositNonce) << 8) | uint72(originChainID);
